...
export const generateAndDownloadPrescriptionPDF = ({
  ...
  language = 'en',
  isOffline = false,
  agronomistName = 'Dr. V. Ramanjaneyulu, Ph.D.',
  agronomistInstitution = 'PJTSAU / ICAR Accredited',
  agronomistRegNo = 'AP-AGRO-2024-8842',
  ...
  try {
    ...
    // Verified Stamp / Badge on Right (Dual AI-Human Clinical Stamp)
    doc.setFillColor(isOffline ? 217 : (isHumanVerified ? 16 : 5), isOffline ? 119 : (isHumanVerified ? 185 : 150), isOffline ? 6 : (isHumanVerified ? 129 : 105));
    doc.roundedRect(pageWidth - margin - 58, 16, 52, 16, 2, 2, 'F');
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(7.5);
    doc.setTextColor(255, 255, 255);
    doc.text(isOffline ? 'OFFLINE FIELD TRIAGE' : (isHumanVerified ? '50% AI + 50% HUMAN' : 'CLINICALLY VERIFIED'), pageWidth - margin - 32, 22.5, { align: 'center' });
    doc.setFontSize(6);
    doc.text(isHumanVerified ? 'Agronomist Signed & Calibrated' : 'PyTorch + Gemini Consensus', pageWidth - margin - 32, 27.5, { align: 'center' });
    ...
    doc.text(`Certified 50% AI + 50% Human: ${agronomistName} (${agronomistInstitution} | Reg: ${agronomistRegNo})`, margin, footerY - 1);
    doc.setFontSize(6);
    doc.setTextColor(148, 163, 184); // Slate 400
    doc.text('Complies with Mandi, ICAR & Digital Agriculture Standards', margin, footerY + 3);
    doc.text('Digital Signature: AGRISHIELD-CLINICAL-VALIDATED', pageWidth - margin, footerY + 1, { align: 'right' });
