...
export default function MultiLeafUploader({
  ...
  const defaultSampleLabels = [
    t('batch_scan.zones.ne', 'North-East Corner'),
    t('batch_scan.zones.center', 'Field Center Plot'),
    t('batch_scan.zones.sw', 'South-West Corner'),
    t('batch_scan.zones.canopy', 'Upper Canopy'),
    t('batch_scan.zones.lower', 'Lower Foliage Near Soil')
  ];
  ...
  const handleFilesChosen = async (e) => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        const label = defaultSampleLabels[slot] || `${t('batch_scan.zone', 'Zone #{{index}}', { index: slot + 1 })}`;
        ...
        onAddSample({
          ...
          label: defaultSampleLabels[slot] || `${t('batch_scan.zone', 'Zone #{{index}}', { index: slot + 1 })}`,
          id: Date.now() + i
        });
      }
    }
    ...
    <Card glass className="p-5 sm:p-6 border-slate-200/80 dark:border-white/10 shadow-xl space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-100 dark:border-slate-800">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-base sm:text-lg font-black text-slate-900 dark:text-slate-100">
                {t('batch_scan.uploader_title', 'Multi-Leaf Plot Sampling (2 to 5 Leaves)')}
              </h3>
              <p className="text-xs text-slate-500 dark:text-slate-400">
                {t('batch_scan.uploader_subtitle', 'Sample leaves from different corners of your field to determine the whole-plot infection rate.')}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          <Badge variant={samples.length >= 2 ? "healthy" : "default"} className="px-2.5 py-1 text-xs font-extrabold">
            {t('batch_scan.leaves_selected', '{{count}} / 5 Leaves Selected', { count: samples.length })}
          </Badge>
          {samples.length > 0 && (
            <button
              type="button"
              ...
              title={t('batch_scan.clear_all', 'Clear all samples')}
            >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
        {[0, 1, 2, 3, 4].map((idx) => {
          ...
          const defaultLabel = defaultSampleLabels[idx];
          ...
          return (
            ...
            >
              ...
              <span className="text-[10px] font-extrabold text-slate-600 dark:text-slate-300 group-hover:text-emerald-600 dark:group-hover:text-emerald-400 block truncate w-full">
                {t('batch_scan.add_leaf', '+ Leaf #{{index}}', { index: idx + 1 })}
              </span>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-2">
        <div className="flex items-center gap-2">
          <button
            ...
            className="px-3 py-2 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-emerald-50 dark:hover:bg-emerald-950/40 text-slate-700 dark:text-slate-200 text-xs font-bold flex items-center gap-1.5 transition-all active:scale-95 disabled:opacity-40 cursor-pointer"
          >
            <Camera className="w-4 h-4 text-emerald-500" />
            <span>{t('batch_scan.take_photo', 'Take Photo')}</span>
          </button>
          ...
          >
            ...
            <span>{t('batch_scan.choose_gallery', 'Choose from Gallery')}</span>
          </button>
        </div>
        ...
        <button
          ...
          className={`px-5 py-2.5 rounded-xl font-black text-xs sm:text-sm flex items-center justify-center gap-2 shadow-lg transition-all active:scale-95 cursor-pointer ${
        ...
        >
          {loading ? (
            <>
              ...
              <span>{t('batch_scan.diagnosing_progress', 'Diagnosing {{count}} Field Samples...', { count: samples.length })}</span>
            </>
          ) : (
            <>
              <Sparkles className="w-4 h-4 text-amber-300" />
              <span>{t('batch_scan.analyze_btn', 'Analyze Entire Field Plot ({{count}} Samples)', { count: samples.length })}</span>
            </>
          )}
        </button>
      </div>
      ...
      {samples.length < 2 && (
        <p className="text-[11px] text-slate-400 text-center font-medium pt-1">
          {t('batch_scan.min_samples_hint', '💡 Please add at least 2 leaf samples (e.g. one from plot edge and one from plot center) to calculate the field infection percentage.')}
        </p>
      )}
    </Card>
  );
